...
#[async_trait]
impl AccountingProvider for TinkConnector {
    fn provider_id(&self) -> &str { "tink" }
    fn display_name(&self) -> &str { "Tink" }
    ...
    async fn health_check(&self) -> Result<ProviderHealth> {
        Ok(ProviderHealth { provider_id: "tink".to_string(), status: HealthStatus::Healthy, latency_ms: 0, message: None })
    }
}
